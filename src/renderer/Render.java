...
public class Render {
	 ...
	 */
	private void castRay(int nX, int nY, int col, int row,double alfa) {
		Ray ray = camera.constructRayThroughPixel(nX, nY, col, row);
		Color color = tracer.traceRay(ray, alfa);
	 ...
	 */
	private void renderImageThreaded(double alfa) {
		...
		for (int i = threadsCount - 1; i >= 0; --i) {
			threads[i] = new Thread(() -> {
				...
				while (thePixel.nextPixel(pixel))
					castRay(nX, nY, pixel.col, pixel.row,alfa);
			});
		}
		// Start threads
	...
	public void renderImage(double alfa) {
		...
		if (threadsCount == 0)
			for (int i = 0; i < nY; ++i)
				for (int j = 0; j < nX; ++j) 		
				 castRay(nX, nY, j, i,alfa);	
		else
			renderImageThreaded(alfa);
	}
